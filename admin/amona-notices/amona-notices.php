...
if ( ! class_exists( 'Amona_Notices' ) ) :
	...
	class Amona_Notices {
		...
		public function __construct() {
			...
			add_action( 'wp_ajax_amona-notice-dismiss', array( $this, 'dismiss_notice' ) );
		...
		public function dismiss_notice() {
			...
			if ( ! apply_filters( 'amona_notices_user_cap_check', current_user_can( $capability ) ) ) {
				return;
			}

			if ( false === wp_verify_nonce( $nonce, 'amona-notices' ) ) {
				wp_send_json_error( esc_html_e( 'WordPress Nonce not validated.', 'amona' ) );
			}
		...
		public function enqueue_scripts() {
			wp_register_style( 'amona-notices', self::get_url() . 'notices.css', array(), self::$version );
			wp_register_script( 'amona-notices', self::get_url() . 'notices.js', array( 'jquery' ), self::$version, true );
			wp_localize_script(
				'amona-notices',
				'amonaNotices',
				array(
					'_notice_nonce' => wp_create_nonce( 'amona-notices' ),
				)
			);
		}
		...
		public function show_notices() {
			$defaults = array(
				'id'                         => '',      // Optional, Notice ID. If empty it set `amona-notices-id-<$array-index>`.
				...
				'display-with-other-notices' => true,    // Should the notice be displayed if other notices  are being displayed from amona_Notices.
				'is_dismissible'             => true,
		...
		public static function markup( $notice = array() ) {
			wp_enqueue_script( 'amona-notices' );
			wp_enqueue_style( 'amona-notices' );

			do_action( 'amona_notice_before_markup' );

			do_action( "amona_notice_before_markup_{$notice['id']}" );

			?>
			<div id="<?php echo esc_attr( $notice['id'] ); ?>" class="<?php echo 'amona-notice-wrapper ' . esc_attr( $notice['classes'] ); ?>" data-repeat-notice-after="<?php echo esc_attr( $notice['repeat-notice-after'] ); ?>">
				<div class="amona-notice-container">
					<?php do_action( "amona_notice_inside_markup_{$notice['id']}" ); ?>
					<?php echo wp_kses_post( $notice['message'] ); ?>
				</div>
			</div>
			<?php

			do_action( "amona_notice_after_markup_{$notice['id']}" );

			do_action( 'amona_notice_after_markup' );
		}
		...
		private static function get_wrap_classes( $notice ) {
			$classes = array( 'amona-notice', 'notice' );
		...
		/**
		 * Get base URL for the amona-notices.
		 *
		 * @return mixed URL.
		 */
